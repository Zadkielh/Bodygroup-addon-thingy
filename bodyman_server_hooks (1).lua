...
function PlayerSpawnBodygroups( ply )
	...
	local job = ply:nut.faction.indices() -- this is the part where it checks for job bodygroups, and applies them if need be
	local skins = {}
	local bodygroups = {}

	timer.Simple(1, function()
		if ply.LastJob_bodygroupr == job.command then
			if table.HasValue( ( job.model or {} ), ( ply.LastPlayermodel or "" ) ) then
